...
**Next up (in order):**
1. Apply migration `supabase/migrations/004_accept_invite_rpc.sql` via Supabase Dashboard SQL Editor (same pattern as 003 / 003b). Verify with `SELECT proname, prorettype::regtype FROM pg_proc WHERE proname = 'accept_garden_invite';` (expect one row).
2. Human interactive tap-through of caretaker Phase 3 — follow `test-results/phase-3-test.md`, which covers: pg_policies sanity check, API-level curl runbook (accept happy path, repeat-accept → already_accepted, non-existent invite → 404, email mismatch → 403, decline via DELETE, manually-expired invite → 410, already-caretaker cleanup behavior), and simulator tap-through on iPhone 17 Pro for the pre-seeded invite addressed to `sam.morassutti+caretaker1@gmail.com`.
3. Human interactive smoke test of caretaker Phase 2 still outstanding on iPhone 17 Pro simulator — follow `test-results/phase-2-test.md` steps A–I.
4. Caretaker mode Phase 4 — `useGardenStore` (activeGardenId persisted to AsyncStorage, filters expired caretaking gardens), refactor `usePlantStore.loadPlants()` to be garden-scoped, Home header dropdown + GardenPickerBottomSheet, caretaker write-gating on HomeScreen (hide + FAB) and PlantDetailScreen (hide Edit/Remove, keep Mark Watered/Fertilized, lock owner notes). First live RLS test.
   - **Owner-facing acceptance signal subtask:** add `garden_caretakers.owner_seen_at` (or equivalent), mark newly-accepted caretakers as unseen, show a badge on `ManageCaretakersScreen` and clear on visit. Deferred from Phase 3 because it fits naturally with Phase 4's owner-side garden-switching work.
5. Caretaker mode Phase 5 — caretaker notes. New `caretakerNotesService.ts` (listNotes/addNote/deleteNote), Caretaker Notes section on PlantDetailScreen with avatar + author + relative timestamp and RLS author-only delete.
6. Caretaker mode Phase 6 — TestFlight build 6 ships caretaker mode. Follow Apr 16 checklist (bump to 6, clean archive to `/tmp/ViraPlantsTemp6.xcarchive`, upload, e2e test with a second Apple ID).
7. Confirm Metro + Notifee on Ninja Sam physical device.
8. Confirm Notifee on physical device via TestFlight.
9. BLE permissions + Phase 2 (Vira pot) wiring.
10. Pre-submission: re-wire Apple Sign-In.
11. **Post-Phase-6 cleanup:** replace owner-side `listMyInvites` / `cancelInvite` with direct PostgREST calls now that migration 003b patched the `garden_invites` RLS policies. Delete `supabase/functions/caretaker-invites/` once nothing invokes it. See the Caretaker Mode Integration Pattern section for context.
...
## Caretaker Mode Integration Pattern (Phases 0–3 complete)

- **Schema** (`supabase/migrations/003_caretaker_mode.sql` + `003b_garden_invites_rls_patch.sql` + `004_accept_invite_rpc.sql`, all applied remotely, 003 + 003b backfilled locally, 004 applied then backfilled): `garden_caretakers` (owner_id, caretaker_id, expires_at), `garden_invites` (owner_id, invitee_email lowercased, token, invite_expires_at default 7d, expires_at for caretaker access, accepted_at), `caretaker_notes` (plant_id, author_id, body). RLS helper `has_garden_access(uuid)` is SECURITY DEFINER. 003b rewrote the `garden_invites` SELECT + DELETE policies to use `auth.jwt() ->> 'email'` instead of subqueries against `auth.users` (see Deployment Learnings). 004 adds `public.accept_garden_invite(uuid, uuid)` — SECURITY DEFINER, EXECUTE granted only to `service_role`, called by the `accept-invite` Edge Function.
...
- **Client service** (`src/services/caretakerService.ts`) — one service for all caretaker APIs.
  ...
  - Caretaker side (Phase 3): `listPendingInvitesForMe` (direct PostgREST SELECT on `garden_invites` + separate SELECT on `profiles` for owner names), `acceptInvite` (Edge Function `accept-invite`), `declineInvite` (direct PostgREST DELETE).
  - Only stub remaining is `listGardensImCaretaking` (Phase 4).
- **`caretaker-invites` Edge Function is a historical workaround** — exists because of the `auth.users` RLS bug fixed in migration 003b. `listMyInvites()` and `cancelInvite()` still route through it for stability; swapping them back to direct PostgREST is a clean small cleanup PR for post-Phase-6 (logged in the Next up list). Phase 3 intentionally does NOT refactor this to avoid churn during the phased rollout.
- **UI surface** — `SettingsScreen` now has a "Pending invitations" section at the top that only renders when `listPendingInvitesForMe()` returns rows; it uses `PendingInviteCard` (Butter Moon, Vermillion Accept / outlined Decline, inline Vermillion error text on failure, Dismiss-only when expired). The rest of the owner-side flow (`ManageCaretakersScreen` → `InviteCaretakerScreen` modal) is unchanged. No real-time subscriptions; list screens refetch on mount + focus + pull-to-refresh.
- **Accept/decline feedback is silent by design.** Successful accept removes the card; no toast. The Phase 4 home-header garden list is the durable confirmation surface — a toast would be a transient shadow of permanent UI. A `TODO(phase-4)` in `SettingsScreen :: handleAcceptInvite` marks where to trigger `useGardenStore.loadGardens()` once that store lands.
- **Phase 3 specifically does NOT add** owner-facing "your invite was accepted" notifications, a generic notifications table, real-time subscriptions, deep linking, auto-deletion of expired invites, or any toast/snackbar infrastructure. The `owner_seen_at` badge on ManageCaretakersScreen is parked as a Phase 4 subtask.
