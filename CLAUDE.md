...
- **RLS on `garden_invites` references `auth.users` (Apr 2026)** — migration 003 shipped a SELECT policy whose USING clause subqueries `auth.users`, which `anon` and `authenticated` can't read. This makes direct PostgREST SELECT/DELETE on `garden_invites` fail with `42501 permission denied for table users` regardless of filters or columns. Worked around by routing owner-side list and cancel through a new `caretaker-invites` Edge Function that uses service role (BYPASSRLS). The right long-term fix is to rewrite the policy to use `auth.jwt() ->> 'email'` instead of the subquery — see `questions.md` Q1 — but that's out of scope for Phase 2. Phase 3 (invite acceptance) should be implemented via service-role Edge Function as well; do not try to SELECT `garden_invites` from the client until the RLS is fixed.
